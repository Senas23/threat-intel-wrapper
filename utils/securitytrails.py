...
class securitytrails():
    ...
    def domain_searcher(self, key_filter, **kwargs):
        """
        ...
        """
        endpoint = '{}/domains/list'.format(self.base_url)
        ...
        if values['filter']:
            ...
            logger.debug(f"{bcolors.OKGREEN}[+] Query for: {values}{bcolors.ENDC}")
            querystr = {"page": "1"}
            r = self.session.request("POST", url=endpoint, json=values, params=querystr)
            domains = {}
            # Loop over multi pages here
            if r.status_code == 200:
              domains = set([str(entry[key_filter]) for entry in r.json()["records"]])
              logger.debug(f"{bcolors.OKGREEN}[+] Query Output Page 1: {r.content}{bcolors.ENDC}")
              if r.json()['meta']['total_pages'] > 1:
                for page in range(1, r.json()['meta']['total_pages']):
                  logger.debug(f"{bcolors.OKGREEN}[+] Query Output Page {page + 1}: {r.content}{bcolors.ENDC}")
                  querystr = {"page": str(page + 1)}
                  time.sleep(1) # API rate limit is 2 per second, so let's be conservative
                  r = self.session.request("POST", url=endpoint, json=values, params=querystr)
                  if r.status_code == 200:
                    for data in r.json()['records']:
                      domains.add(data[key_filter])
            logger.debug(f"{bcolors.OKGREEN}[+] Output: \n{domains}{bcolors.ENDC}")
